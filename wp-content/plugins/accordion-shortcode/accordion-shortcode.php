<?php
/*
Plugin Name: Creative Marketing Accordion Shortcode
Plugin URI: http://creativem.co.nz
Description: Adds shortcode that enables you to create accordions. Based on accordion short_code but added a few extra elements
...
/**
 * KP_Accordion_Shortcode class.
 */
class KP_Accordion_Shortcode {
	...
	public static  function accordion_shortcode( $atts, $content ) {
		...
		if($title): ?>
			<h3 id="<?php echo ereg_replace("[^A-Za-z0-9]", "", $title)."-".self::$shortcode_count; ?>" >
                            <a href="#<?php echo ereg_replace("[^A-Za-z0-9]", "", $title)."-".self::$shortcode_count; ?>"><?php echo $title; ?></a>
                        </h3>
...
// lets play
KP_Accordion_Shortcode::init();

//Oleg Shortcodes AryGeek-------------------------------------------------
add_shortcode('links-popup', 'popup_function');

add_shortcode('usa', 'usa_function');
add_shortcode('europe', 'europe_function');
add_shortcode('pacific', 'pacific_function');

add_shortcode('end', 'end_div_function');
add_shortcode('end-links-popup', 'end_links_popup_function');


function popup_function( $atts ) {
	extract( shortcode_atts( array(
		'id' => 'enter-id-for-popup-functionality',
	), $atts, 'bartag' ) );

	return '<div class="popup-container" id="' . $id . '">';
}
function usa_function() {
  return '<div class="usa-box">';
}
function europe_function() {
  return '<div class="europe-box">';
}
function pacific_function() {
  return '<div class="pacific-box">';
}

function end_div_function() {
  return '</div>';
}
function end_links_popup_function() {
  return '</div>';
}
